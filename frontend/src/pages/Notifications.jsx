  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <header className="flex items-end justify-between">
        <div>
          <div className="overline text-[#002FA7]">— Inbox</div>
      ...
      <Card className="rounded-sm border border-gray-200 shadow-none p-0 bg-white divide-y divide-gray-100">
        ...
        {items.map((n) => (
          ...
          >
            <div className={`mt-1 w-2 h-2 rounded-full ${!n.read ? "bg-[#002FA7]" : "bg-transparent"}`} />
            <div className="flex-1">
              ...
              {n.link && <Link to={n.link} className="text-xs text-[#002FA7] hover:underline mt-2 inline-block">Open →</Link>}
            </div>
          </div>
        ))}
      </Card>
